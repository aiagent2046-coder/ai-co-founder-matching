...
class SyndiBot:
    ...
    async def signup(self) -> bool:
        ...
        try:
            ...
            if r.status_code in (200, 201):
                ...
                return True
            elif r.status_code == 422 and "already registered" in r.text.lower():
                logger.info(f"[{self.profile['name']}] Already registered, logging in...")
                return await self.login()
            else:
                logger.error(f"[{self.profile['name']}] Signup failed: {r.status_code} {r.text[:200]}")
                return False
        except Exception as e:
            logger.error(f"[{self.profile['name']}] Signup exception: {e}")
            return False
    ...
    async def _api_post(self, path: str, payload: dict) -> Optional[dict]:
        """POST к API SyndiAI с авторизацией."""
        client = await self._client_()
        headers = {
            "Content-Type": "application/json",
        ...
        try:
            r = await client.post(f"{self.app_url}{path}", json=payload, headers=headers)
            if r.status_code in (200, 201):
                return r.json()
            else:
                logger.warning(f"[{self.profile['name']}] API POST {path} -> {r.status_code}: {r.text[:200]}")
                return None
        except Exception as e:
            logger.error(f"[{self.profile['name']}] API POST {path} exception: {e}")
            return None

    async def _api_get(self, path: str) -> Optional[dict]:
        """GET к API SyndiAI с авторизацией."""
        client = await self._client_()
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                logger.warning(f"[{self.profile['name']}] API GET {path} -> {r.status_code}: {r.text[:200]}")
                return None
        except Exception as e:
            logger.error(f"[{self.profile['name']}] API GET {path} exception: {e}")
            return None
    ...
    async def _save_profile_direct(self, payload: dict) -> bool:
        ...
        try:
            ...
            if r.status_code in (200, 201):
                ...
                logger.info(f"[{self.profile['name']}] Profile saved directly")
                return True
            return False
        except Exception as e:
    ...
    async def get_candidates(self) -> list[dict]:
        ...
        if result and "candidates" in result:
            ...
            return candidates
        logger.warning(f"[{self.profile['name']}] No candidates from API")
        return []
    ...
    async def run_discover_session(self, max_swipes: int = 20) -> int:
        ...
        for candidate in candidates[:max_swipes]:
            await self.swipe(candidate)
            swiped += 1
            await asyncio.sleep(random.uniform(0.5, 1.5))  # Имитация задумчивости

        logger.info(f"[{self.profile['name']}] Session complete: {swiped} swipes, {len(self.state.matches)} total matches")
        return swiped
    ...
    async def run_discover_and_chat(self, sessions: int = 3) -> None:
        ...
        logger.info(f"[{self.profile['name']}] Starting discover sessions ({sessions})")
    ...
    async def _save_state(self) -> None:
        ...
        import os
        state_file = f"/mnt/agents/output/syndi-agents/profiles/bot_{self.profile['index']:03d}_state.json"
        os.makedirs(os.path.dirname(state_file), exist_ok=True)
        state = {
            "profile": {k: v for k, v in self.profile.items() if k != "password"},
            "user_id": self.state.user_id,
