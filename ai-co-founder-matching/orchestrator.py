...
class Orchestrator:
    ...
    def generate_profiles(self, count: int = 100) -> list[dict[str, Any]]:
        ...
        # Сохраняем
        os.makedirs("/mnt/agents/output/syndi-agents/profiles", exist_ok=True)
        with open("/mnt/agents/output/syndi-agents/profiles/all_profiles.json", "w", encoding="utf-8") as f:
            # Убираем пароли из сохранения
    ...
    def _save_csv(self, profiles: list[dict]) -> None:
        ...
        import csv
        csv_path = "/mnt/agents/output/syndi-agents/profiles/overview.csv"
    ...
    async def run(self, profiles: list[dict[str, Any]]) -> RunStats:
        ...
        for result in results:
            ...
            if result.get("status") == "success":
                ...
                self.stats.total_messages += result.get("conversations", 0)
            else:
                self.stats.failed += 1
                if "error" in result:
