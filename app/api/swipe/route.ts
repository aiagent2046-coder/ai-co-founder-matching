...
async function generateAutoReply(
  ...
  const prompt = `Ты — AI-аватар фаундера по имени ${me.name} (роль: ${me.role}). Пишешь ПЕРВОЕ сообщение новому контакту по имени ${them.name} (роль: ${them.role}${them.domain ? ', сфера: ' + them.domain : ''}), с которым только что случился взаимный мэтч. Обратись К СОБЕСЕДНИКУ по имени ${them.name} (не к себе!), пиши от первого лица как ${me.name}, кратко и дружелюбно (1–2 предложения), предложи обсудить сотрудничество. Не упоминай, что ты ИИ. Ответь ТОЛЬКО текстом сообщения, без кавычек.`;
