  ...
  if (role.id === 'engineer') {
    ...
    if (ghToken) {
      const toolSystem = systemPrompt +
        ...
        '\n\nБЮДЖЕТ ЧТЕНИЯ: у тебя ОГРАНИЧЕННОЕ число шагов с инструментами. ' +
        'НЕ пытайся прочитать весь проект. Прочитай НЕ больше 4–5 КЛЮЧЕВЫХ файлов ' +
        '(сначала дерево/README/конфиги, потом 2–3 самых важных исходника), затем ДАЙ АНАЛИЗ. ' +
        'Не откладывай вывод ради "ещё одного файла". Если для полноты нужно больше — ' +
        'скажи пользователю сузить запрос (конкретная папка/файл) следующим сообщением.' +
